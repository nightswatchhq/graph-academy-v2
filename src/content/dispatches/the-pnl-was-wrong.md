...
Before shipping it we compared the new daily view, running on the production nest, against that community subgraph for every indexer and every day from 24 July to 22 August: 327 indexer-days, seven figures each (total, indexer and delegator rewards, the number of reward collections, gross fees, the curators' share, and the number of fee collections). Every one of them matches to the wei.
...
```sql
SELECT day_start, indexer_rewards, delegator_rewards, fees_gross, fees_net
FROM lodestar_indexer_daily
WHERE indexer = '0x…' AND day_start >= epoch(current_date - INTERVAL 30 DAY)
ORDER BY day_start;
```

The work is in graph-allocations-nest#23, kittiwake#140 and #142, and lodestar#229 and #230.

If your numbers and ours disagree, tell us. This time we would like to hear about it first.
