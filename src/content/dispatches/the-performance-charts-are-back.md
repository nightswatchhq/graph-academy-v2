...
- **The populated charts in a browser.** An early check against a private preview showed the empty states only. On 15 September, after the rebuild, the Lodestar branch was run locally against production data and read in a browser. For pinax2.eth all six Query Performance cards drew the full window: 148.6M queries, 100,063.37 GRT in fees, 99.72% success with the 1 to 3 July silence drawn as a gap, 197 ms latency beside Foghorn's probes, and about 33 seconds behind the freshest peer, measured on 91.9% of its queries. p2p-org-booster.eth's charts stop on 15 July, when the gateway stopped sending it traffic, and its QoS Quality panel says it has no score for the last 30 days rather than showing one. That is what verified means here: read by eye on two indexers, not every indexer's page.
