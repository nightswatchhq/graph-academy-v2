...
- **The 90-day figures on production.** <!-- TODO(release): the rebuilt 90-day figures once the rebuild running now completes: settled days, stored indexer-days and indexers in the window, the whole window's count of empty and malformed documents, and any day that does not settle and why. -->
...
**The first visitor waits.** Settled days come from Postgres, but today comes from the nest, live, so the first visit to an indexer page after the cache empties waits on it. When kittiwake shipped, the QoS route took 8.9 s cold; later that day it waited out its 45-second timeout. kittiwake#148, which keeps every directory indexer's page warm, is written and not merged, and its extra load on the nests is unmeasured. <!-- TODO(release): route latency for /qos, /qos-score and /qos-deployments, cold and warm, after the rebuild. -->
